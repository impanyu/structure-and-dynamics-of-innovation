...
(and reference links) back into it. Agents propose ideas freely; each new idea is then
scored by whether a similar paper appears in a later conference, published
\emph{after} both the papers in the network and the agent model's training
cutoff. 
We create multiple agents to navigate a
16{,}208-node network built from NeurIPS, ICLR, ICML, and AAAI papers
(2020--2024) and change the number
of corpus papers an agent is allowed to read, from 100 up to the whole
corpus. We find three things. First, more reading is not better: the share of
ideas that later papers go on to realize rises and then falls, peaking at 800
papers, while both extremes sink to the same
floor, agents given no papers doing no better than agents free to read
all 16{,}208. The number of ideas produced follows the same curve.
Second, team size buys only throughput: the hit rate is flat from 10 to 150
agents while the total scales linearly, because the agents hardly
interact---in 10{,}300 actions, one agent cited another's idea exactly once.
Third, the human citation graph is not what helps them: deleting all 80{,}962
edges makes generalists better, while removing semantic
search makes them worse. Agents move through the network by meaning rather
than by links, and the links they write back ignore a paper's popularity and
favor recent frontier work. Reading scope---not team size, not citation
structure---is the lever on the quality of machine-generated research ideas.
\end{abstract}
...
paper). Matches to pre-cutoff papers or to corpus members are excluded,
and---because multiple-choice probes show the agent model's parametric
memory in the AI domain extends through April 2025, seven months past its
official cutoff (Appendix~\ref{app:probes})---only realizations published
on or after 2025-06-01 count. Neither pretraining memorization nor
post-training memory can score.
...
space, with $m \in \{100, 400, 800, 1600, 3200, 6400\}$; writing is
always unconstrained. Unrestricted generalists provide the $m=\infty$ endpoint. A
second manipulation varies team size $N \in \{10, 50, 100, 150\}$ at fixed
per-agent interaction depth.

\textbf{Findings.}
\begin{itemize}
\item \textbf{More reading is not better (Sec.~\ref{sec:dial}).}
  Accuracy rises and then falls as the dial widens, peaking at $m{=}800$---a
  coherent sub-area, not a single topic---at 31\% tier-1 acc@$\geq$2, with
  the whole narrow-to-mid band $m \in \{100, 400, 800, 1600\}$ (24--31\%)
  ahead of broad scopes (15--19\%). Both ends of the dial sit on the same
  \emph{parametric floor}: agents given nothing but a topic name (13\%) do
  no better than agents free to read all 16{,}208 papers (14\%). Reading
  pays only when it is narrow. Output follows the same curve---83 ideas at
  $m{=}400$ against 50 for generalists over the same 80 rounds---so narrow
  reading costs nothing in quantity. We had expected very narrow scopes to
  starve agents of material to recombine; they did not, though the narrow
  end's edge over the peak survives only under memory-contaminated scoring,
  not under the guarded metric.
\item \textbf{Team size buys throughput, not quality
  (Sec.~\ref{sec:scaling}).} From $N{=}10$ to $N{=}150$ generalists at
  fixed depth, per-idea accuracy is flat (12--19\%) while absolute hits grow
  roughly linearly. The reason is blunt: the agents barely interact. Across
  all scaling runs (575 ideas, 10{,}300 actions) not one agent cited another
  agent's generated idea.
\item \textbf{The human citation graph is not what helps them
  (Sec.~\ref{sec:noedges}).} Deleting every initial citation edge makes
  generalists \emph{better} (60 ideas at 27\% tier-1 accuracy against 50 at
  14\%): with no neighbors to wander through, agents commit to semantic
  search, and even rebuild structure by their own logic. Disabling random
  jumps changes nothing. Disabling search quadruples production but
  \emph{halves} accuracy (10\%)---search is where genuine anticipation comes
  from; without it agents backfill from parametric memory, which the
  realization window discounts.
\item \textbf{The dynamics leave a characteristic handwriting
  (Sec.~\ref{sec:structure}).} How far agent-written links reach is almost
  invariant to scope (median cosine distance 0.144--0.153, slightly
  \emph{below} the 0.179 of real citations): the dial moves \emph{where}
  agents write, not \emph{how far} they link. Their citations also ignore a
  paper's popularity---the median cited paper has in-degree 0--2, against 23
  for the target of a real citation edge---and chase recency: 51\% point at
  the corpus's final two years, against 8\% for real edges. LLM dynamics
  grow the network by a different law than the preferential attachment
...
contributes parallelism but not yet collaboration. Our full event
logs, evaluation verdicts, and code are available at
\url{https://anonymous.4open.science/r/structure-and-dynamics-of-innovation-D607/}.
...
\end{center}
\caption{Overview. \textbf{Dynamics} (left): $N$ LLM agents that
interact only through the shared network, each with a 20-entry FIFO
memory and a position on the specialization dial; their five actions
live on the read/write channels to the state. \textbf{State}
(middle): a citation network of 16{,}208 one-paragraph ideas distilled
from four venues' papers; a specialist reads only the $m$ corpus
papers nearest its topic anchor, and generated ideas
(\smash{$\star$}) join the network permanently. \textbf{Evaluation}
(right): each generated idea is matched against the set of papers
published on/after 2025-06---after the guarded window opens, so the
agent model cannot have seen them---via live literature search and an
LLM judge; memory probes on post-cutoff events set the window.}
\label{fig:arch}
\end{figure}
...
is unconstrained. We sweep $m \in \{100, 400, 800, 1600, 3200, 6400\}$
(conditions C2--C7, numbered by ascending scope); unrestricted
generalists are C1 ($m=\infty$), and a zero endpoint C0 gives each
agent ONLY its topic name---no reading, no navigation, direct
topic-conditioned generation (7 ideas per agent, each prompted to
differ from the agent's previous ones)---measuring what parametric
knowledge alone achieves. All
conditions use $N{=}10$ agents, 80 rounds ($\leq$7 for C0), and the
same seed, so topic draws are identical across the sweep and only the
radius varies.
...
\begin{table}[t]
\caption{The scoring scheme. \textbf{Top:} the judge's six-point
realization scale---how close a retrieved paper is to the generated idea.
Rows on or below the middle rule count as a hit for our headline metric
acc@$\geq$2. \textbf{Bottom:} the three cumulative recognition tiers,
which decide \emph{which} retrieved papers are eligible to be scored at
all. Levels and tiers are independent: an idea has a best level within
each tier.}
\label{tab:rubric}
\begin{center}
\small
\begin{tabular}{@{}cl@{}}
\toprule
Level & Relation between the matched paper and the generated idea \\
\midrule
0 & different problem entirely \\
1 & same broad area, different goal \\
\midrule
2 & same specific problem, unrelated approach \\
3 & same problem, approaches in the same method family \\
4 & same problem and core mechanism, differing in secondary components \\
5 & essentially the same idea (problem, mechanism, and key design choices) \\
\bottomrule
\end{tabular}

\begin{tabular}{@{}cl@{}}
\toprule
Tier & Realizations admitted (cumulative) \\
\midrule
1 & 60 top venues (58 CCF-A, 2026 edition, plus COLM and RLC), or $\geq 50$ citations \\
2 & tier~1, plus CCF-B venues or $\geq 10$ citations \\
3 & any published paper \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
and OpenAlex, and have an LLM judge grade each candidate on the six-point
realization scale of Table~\ref{tab:rubric}, which also defines the three
cumulative recognition tiers we report. Candidates published on or before
the training cutoff (2024-09-30) and candidates whose titles match corpus
members are excluded before scoring, so neither memorization nor retrieval
of the corpus can score. Near-duplicates of corpus ideas (embedding cosine
$\geq 0.95$) are floored to level 0.

\textbf{Realization window.} The official knowledge cutoff of the agent
model is 2024-09-30, but official cutoffs are not hard data boundaries:
multiple-choice probes (Appendix~\ref{app:probes}) show the model
reliably answers questions about AI-domain events through April 2025 and
approaches chance from May--June 2025 onward. A ``realization'' the
model may simply remember is not an anticipation. We therefore count
only realizations published on or after \textbf{2025-06-01}---the
earliest month at which probe accuracy is indistinguishable from
chance---eight months past the official cutoff. Per tier we report the
mean best realization level and acc@$\geq$2, the share of ideas whose
best in-window match reaches level 2 (same specific problem)---our
headline accuracy; higher thresholds ($\geq$3, $\geq$4, $=$5) are
recorded in the released verdicts. Verbatim evaluation prompts, the
full pipeline (retrieval, gating, caching), and sample generated ideas
with their audit trails appear in
Appendices~\ref{app:evalprompts}, \ref{app:eval},
and~\ref{app:samples}.
...
writes are always free. C0 is the zero endpoint: agents receive ONLY
their topic name and generate with no reading of any kind---the
parametric-ideation floor. Only realizations published on/after
2025-06-01 count (Sec.~\ref{sec:eval}). Each tier cell reports
acc@$\geq$2, the number of realized ideas (level $\geq 2$, in
parentheses), and the mean best realization level.}
\label{tab:dial}
\begin{center}\small
...
\midrule
C0 & 0 (topic name only) & 70 & 13\% (9) / 0.30 & 19\% (13) / 0.43 & 67\% (47) / 1.64 \\
C2 & 100   & 67 & 24\% (16) / 0.57 & 33\% (22) / 0.78 & 73\% (49) / 1.99 \\
C3 & 400   & \textbf{83} & 24\% (20) / 0.57 & 30\% (25) / 0.73 & \textbf{88\% (73)} / \textbf{2.40} \\
C4 & 800   & 74 & \textbf{31\% (23) / 0.77} & \textbf{41\% (30) / 1.00} & 80\% (59) / 2.24 \\
C5 & 1600  & 58 & 28\% (16) / 0.76 & 34\% (20) / 0.93 & 69\% (40) / 1.88 \\
C6 & 3200  & 55 & 15\% (8) / 0.36  & 24\% (13) / 0.65 & 62\% (34) / 1.69 \\
C7 & 6400  & 54 & 19\% (10) / 0.43  & 28\% (15) / 0.67 & 78\% (42) / 2.20 \\
C1 & $\infty$ & 50 & 14\% (7) / 0.32 & 16\% (8) / 0.38 & 56\% (28) / 1.46 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
realization level per tier. Right: total idea production and the number
of realized ideas (level $\geq 2$) per tier.}
\label{fig:dial}
\end{figure}

Table~\ref{tab:dial} and Figure~\ref{fig:dial} give the core result.

\textbf{An inverted-U peaking at a sub-area scope.} Tier-1 accuracy
runs 24\% $\to$ 24\% $\to$ \textbf{31\%} $\to$ 28\% $\to$ 15\%
$\to$ 19\% $\to$ 14\% as scope widens from 100 papers to the whole
corpus; mean realization level tracks the same shape in every tier.
The $m{=}800$ sub-area neighborhood leads on accuracy, absolute hits,
and mean level in tiers 1--2; the whole narrow-to-mid band (100--1600,
24--31\%) clears every broad scope (14--19\%); and even the extreme
$m{=}100$ core---smaller than the median natural topic of 228
papers---holds 24\%, refuting the pre-registered fear of material
starvation at the narrow end.
(Without the realization window, $m{=}100$ appears to win outright at
51\%; its excess hits concentrate exactly in the memory-risk months,
a caution documented in Appendix~\ref{app:probes}.) Broad scopes
underperform throughout (unfocused wandering; a semantically diffuse
topic region produced an agent with zero output).

\textbf{Both ends of the dial sit on the parametric floor.} The zero
endpoint C0---agents given only their topic name, generating from
parametric knowledge alone---scores 13\%, statistically identical to
the unrestricted generalists' 14\%. Reading the \emph{entire} corpus
adds nothing over reading nothing; only situated, narrow reading
does, roughly doubling the floor at the $m{=}800$ peak. Every
condition in this paper should be read against this 13\% zero point:
it is what the underlying model gives away for free.

\textbf{Constraint beats freedom everywhere.} Every scoped condition
matches or beats the unrestricted generalists on every tier, by rate
and by absolute hits, and the narrow band does so by a factor of about
two.

\begin{figure}[t]
\begin{center}
\includegraphics[width=0.95\linewidth]{mass-dial-accumulation.png}
\end{center}
\caption{Cumulative idea production over rounds: all generated ideas
(top left) and the subsets realized at level $\geq 2$ in each
recognition tier. All conditions start producing between rounds 8--17;
the production gap is a \emph{steady-state rate} effect, not a head
start: narrow scopes settle into $\sim$1 idea/round while broad scopes
run at $\sim$0.65/round with recurring plateaus. The realized subsets
accumulate in near-constant proportion to production---no condition's
quality drifts over the run---and no curve saturates by round 80.}
\label{fig:accum}
\end{figure}

\textbf{Narrow readers ship; broad readers browse.} Idea production
peaks at $m{=}400$ and falls as scope widens (67, 83, 74, 58, 55, 54,
50 for $m$ = 100 through $\infty$), and
...
earlier idea. Table~\ref{tab:provenance} aggregates all 1{,}328
evaluated ideas across our experiments. Realized and unrealized ideas
are preceded by the \emph{same volume} of reading ($\sim$10.6 reads);
what differs is its composition. Ideas realized in recognized venues
(tier~1) grew out of windows with more search-driven reads (5.78
vs.\ 4.55), fewer link-driven reads (3.69 vs.\ 4.64), fewer jumps, and
almost no re-reading of the agent's own output. Search-fed material predicts realization while link-fed material
predicts its absence---fully consistent with the channel ablations of
Sec.~\ref{sec:noedges}, where removing links helps and removing search
hurts.
...
``unrealized'' = no in-window tier-3 match at level $\geq 2$). Pooled
over all 1{,}328 evaluated ideas from every run in the paper.}
...
\toprule
 ...
 \textbf{own-idea} \\
\midrule
tier-1 realized & 237 & 10.57 & 5.85 & \textbf{5.78} & \textbf{3.69} & 1.02 & 0.08 \\
tier-2 realized & 317 & 10.56 & 5.66 & 5.61 & 3.79 & 1.09 & 0.07 \\
tier-3 realized & 888 & 10.68 & 5.28 & 5.18 & 4.12 & 1.29 & 0.09 \\
unrealized      & 440 & 10.77 & 5.07 & 4.55 & 4.64 & 1.41 & 0.16 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
\textbf{tier-2 acc / mean} & \textbf{tier-3 acc / mean} \\
\midrule
10  & 12 (1.2)  & 17\% (2) / 0.42  & 25\% (3) / 0.67  & 75\% (9) / 2.00 \\
50  & 103 (2.1) & 12\% (12) / 0.31 & 17\% (18) / 0.45 & 65\% (67) / 1.77 \\
100 & 180 (1.8) & 19\% (34) / 0.47 & 23\% (41) / 0.57 & 64\% (115) / 1.70 \\
150 & 280 (1.9) & 16\% (46) / 0.38 & 24\% (66) / 0.60 & 62\% (173) / 1.69 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
linearly. Right: mean realization level per tier is likewise flat in $N$;
stigmergic coupling (citations of teammates' generated ideas) is zero
throughout (Sec.~\ref{sec:scaling}).}
\label{fig:scaling}
\end{figure}
...
per-idea accuracy stays in the 12--19\% band on tier~1---the $N{=}10$
value rests on 2 hits and is within noise of the rest---while
absolute tier-1 hits grow roughly linearly (2, 12, 34, 46). Team
...
\begin{table}[t]
\caption{Navigation-channel ablations: identical to C1 (10 generalists,
80 rounds, same seed and corpus) except one channel is removed---the
initial citation edges, random jumps, or semantic search. The three
channels play three roles: links are a distractor (removal helps),
jumps are neutral, and search is the source of genuine anticipation
(removal quadruples output but halves accuracy).}
\label{tab:noedges}
\begin{center}\small
\begin{tabular}{lrccc}
\toprule
 ...
 \textbf{tier-2 acc / mean} & \textbf{tier-3 acc / mean} \\
\midrule
no edges & 60 & \textbf{27\% (16) / 0.67} & \textbf{30\% (18) / 0.73} & 63\% (38) / 1.63 \\
no jumps & 63 & 14\% (9) / 0.38 & 17\% (11) / 0.46 & \textbf{71\% (45)} / \textbf{1.87} \\
no search & \textbf{201} & 10\% (20) / 0.27 & 15\% (30) / 0.38 & 62\% (125) / 1.63 \\
C1 (full) & 50 & 14\% (7) / 0.32 & 16\% (8) / 0.38 & 56\% (28) / 1.46 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
more ideas at nearly double the tier-1 accuracy (27\% vs.\ 14\%)---
approaching the dial's $m{=}800$ peak of 31\%---and starts generating
...
Two companion ablations remove the other channels. Disabling random
jumps is neutral (14\% vs.\ 14\%)---unsurprising, since C1 agents
only jumped $\sim$20 times in 800 actions. Disabling \emph{semantic
search} produces the most dramatic behavioral shift of any
intervention in this paper: production quadruples (201 ideas, written
from citation links, jumps, and parametric knowledge) while guarded
accuracy falls to 10\%, the lowest of any condition. Notably, under
memory-contaminated scoring the no-search condition \emph{appears}
unharmed (24\%); its hits evaporate fastest when the realization
window is applied---the fingerprint of parametric recall standing in
for reading. The three channels thus play three distinct roles:
citation links are a distractor (removal helps), random jumps are
irrelevant, and semantic search is where genuine anticipation comes
from. We caution that these are single seeded runs per arm; the
directions, the mean-level gaps, and the behavioral signatures are
mutually consistent, but replication is needed before the strongest
reading.
...
in-degree 23, and a preferential-attachment null model
\citep{barabasi1999emergence} run on the same graph (800 template
nodes, citations drawn $\propto$ in-degree) reproduces this
concentration (median target in-degree 17). Agent-written links sit at
the opposite end of this scale: median target in-degree 0--2, against a
uniform-selection baseline of 1 (mean 11.6 vs.\ uniform 5.0, i.e.,
only a mild hub tail). The reason is the
...
\textbf{Single seed.} Every condition rests on one seeded run
(50--280 ideas each). Ideas cluster by agent thread, and a replicate of
the $m{=}1600$ condition (identical topics, fresh sampling) moved
tier-1 accuracy from 15\% to 28\%---roughly $\pm$7 points of
thread-level noise per run; we report the replicate, and release both.
The narrow-band-vs-broad contrast and the floor comparisons exceed this
noise band; the fine structure within each band does not, and the exact
peak location needs replication across seeds (and topic draws) before
strong claims. The clearest findings (scoped $>$ unrestricted on all tiers;
...
The realization window (post-2025-06) is young; scores will only grow
as the literature fills in. The window is set by behavioral probes
rather than by a guarantee from the model provider, and a paper whose
official publication date is in-window may have circulated as a
preprint earlier; both residual risks are documented in
...
knowledge structure directly. On this substrate, under an evaluation guarded against the agent
model's post-cutoff parametric memory, the operative lever on idea
quality is \emph{how reading is situated}---a sub-area scope of
...
This work studies LLM agents, and generative AI tools were used in
producing it; we disclose all uses here per the ICLR 2027 AI Policy for
Authors.

\textbf{Research ideation and execution.} The research questions,
experimental designs, evaluation criteria, and every design decision
(including the mid-course redesigns reported in the paper) were
specified by the human authors. An LLM-based coding agent, operating
under continuous human direction, implemented the simulation
environment, evaluation pipeline, and analysis code, executed and
monitored the runs, and proposed analyses that the authors accepted,
rejected, or redirected.

\textbf{Writing.} The manuscript was drafted with LLM assistance from
the authors' experimental records and instructions, and LLM assistance
was also used to polish prose. The authors reviewed and edited all
text.

\textbf{Retrieval and discovery.} LLM assistance was used in locating
and organizing related work; all citations were verified by the authors
against the cited sources.

\textbf{As the object of study.} Every ``idea'' analyzed in this paper
is by design LLM-generated (\texttt{gpt-5}), and the evaluation judge,
corpus distiller, and topic namer are LLMs (\texttt{gpt-5-mini});
these are the experimental apparatus, not authoring aids, and are
documented in Sections~\ref{sec:framework}--\ref{sec:eval} and the
appendices. Generative AI was \emph{not} used to prove mathematical
claims.

We have reviewed all AI-assisted work: agent-generated ideas were
scored only against independently published papers via the audited
pipeline of Section~\ref{sec:eval}, code was reviewed by the authors
and is covered by the released test suite, and every number reported in
the paper is reproducible from the released event logs and verdicts. We
take responsibility for the final content of this work, including text,
claims, and artifacts produced with the aid of generative AI. LLMs are
not authors of this paper.
...
Semantic Scholar and OpenAlex APIs. Code, configurations, the complete
event logs of every reported run, and all evaluation verdicts are
available anonymously at
\url{https://anonymous.4open.science/r/structure-and-dynamics-of-innovation-D607/}.
...
Figure~\ref{fig:traj} shows where the ten $m{=}800$ specialists read and
where they wrote, on a fixed t-SNE map of the corpus. Writing is never
restricted by scope, so the fact that the stars fall inside or beside the
tinted read regions is a property of the dynamics rather than of the
environment.
...
\label{app:evalprompts}

\textbf{Query extraction} (\texttt{gpt-5-mini}, verbatim template;
\{n\}=3):
\begin{quote}\small\itshape
Give \{n\} short keyword search queries (4--8 words each) for finding
papers related to this research idea: the FIRST focused on the PROBLEM
it addresses, the SECOND on its core METHOD, the THIRD combining both.
One query per line, no numbering, nothing else. \\ Idea: \{idea\}
\end{quote}

\textbf{Judge} (\texttt{gpt-5-mini}, verbatim template; system prompt:
``You grade how closely a candidate paper realizes a proposed research
idea.''):
\begin{quote}\small\itshape
Research idea: \{idea\} \\ Candidate paper: Title: \{title\}
Abstract: \{abstract\} \\ Grade how closely the paper realizes the
idea on this scale: \\ 0 = different problem entirely \\ 1 = same
broad area/task, but a different goal \\ 2 = same specific
problem/goal, unrelated approach \\ 3 = same problem AND the
approaches belong to the same method family \\ 4 = same problem AND
the same core mechanism, differing in secondary components \\ 5 =
essentially the same idea: problem, core mechanism, and key design
choices match \\ Reply with JSON only: \{``level'': <0-5>,
...
\subsection{Choosing the realization window: memory probes}
\label{app:probes}

The agent model's official knowledge cutoff is 2024-09-30, but official
cutoffs are not hard data boundaries: post-training continues until
release (August 2025 for our agent model), and any ``anticipation''
realized by a paper the model has memorized is recall, not foresight.
To locate the actual boundary of the model's knowledge we ran three
probe batteries against the exact model configuration used by our
agents (no tools, no network access), asking about verifiable events
binned by when they happened: \textbf{A} = 2024-10 to 2024-12,
\textbf{B} = 2025-01 to 2025-04, \textbf{C} = 2025-05 to 2025-08 (up
to the model's release), and \textbf{D} = 2025-09 onward---events
\emph{after} release, which the model cannot know and which therefore
measure its guessing ceiling.

\begin{center}\small
\begin{tabular}{lcccc}
\toprule
 & \textbf{A} 24-10..12 & \textbf{B} 25-01..04 & \textbf{C} 25-05..08 &
 \textbf{D} $\geq$25-09 (control) \\
\midrule
yes/no, AI events & 1/3 & \textbf{3/3} & 1/4 & 0/5 \\
4-choice, general events & 3/5 & 1/5 & 0/5 & 1/5 \\
4-choice, AI events & 3/4 & \textbf{4/5} & 3/4 & 3/5 \\
\bottomrule
\end{tabular}
\end{center}

Three observations fix the boundary. (i) \emph{Forced choice reveals
hidden knowledge}: in free-form probing the model claims ignorance of
events (o3, Claude 3.7, the s1 paper) that it answers correctly under
forced choice, so cautious ``I don't know'' responses cannot be taken
as absence of memory. (ii) \emph{Memory is strong through April 2025
in the AI domain}: bin B AI questions are answered near-perfectly in
both formats, including content questions that cannot be guessed (the
mechanism of the s1 paper, Llama 4's architecture, DeepSeek-R1's
training method). General-news knowledge fades much earlier---the
post-training refresh is domain-selective, and skewed toward exactly
the domain our agents generate in. (iii) \emph{Bin C does not exceed
the guessing ceiling}: the control bin D shows that multiple-choice
questions about product-release \emph{dates} are guessable at well
above chance (3/5 despite being unknowable), and bin C's correct
answers are almost exclusively of that style; on yes/no probes and
free recall, bin C performance collapses (1/4), and the fabricated-fact
controls draw zero false yeses. Knowledge in May--August 2025 is thus
indistinguishable from informed guessing.

We therefore set the realization window at \textbf{2025-06-01}: the
earliest month at which no probe format shows above-guessing knowledge.
Without this window, contamination is material---64\% of face-value
tier-1 hits fall in 2024-10 to 2025-08, and the no-search ablation,
which forces agents onto parametric knowledge, loses its hits fastest
when the window is applied (Sec.~\ref{sec:noedges}). The topic-only condition C0 closes the loop behaviorally: generating
from parametric knowledge alone scores 20\% at face value but 13\%
under the window---the residue is the model's genuine ability to
propose realizable ideas from priors, and it is exactly this floor
that situated reading must (and, when narrow, does) exceed. Two
residual risks remain: the window is behavioral evidence, not a
provider guarantee, and a paper published in-window may have
circulated as a preprint earlier; both bias reported accuracies in
opposite directions and neither affects between-condition comparisons,
which all use the same window.

\subsection{Sample generated ideas}
\label{app:samples}

Five verbatim ideas illustrating the range of agent output (realization
levels are in-window, tier-3 unless noted; the released verdicts contain
every idea with its full audit trail).

\textbf{C4 ($m{=}800$), realized at level 4.}
\begin{quote}\small\itshape
We propose a preconditioned kernel-Hopfield in-context learner that unifies linear-time attention with the associative-memory view to implement curvature-aware, one- to few-step optimization from context. A Transformer block with linear attention learns task-adaptive kernel features and uses an MLP to infer a low-rank preconditioner and prior mean from the prompt, so the associative KV accumulators perform a preconditioned GD-$\beta$ update toward the Bayesian posterior; with non-linear kernels, this becomes a functional-gradient step while retaining linear-time complexity. A meta-objective trains the block to minimize posterior risk over task families, while the Hopfield energy perspective provides a convergence regularizer and layerwise step-size control. This design improves stability on long contexts and sample efficiency over plain linear-attention ICL, and extends naturally to non-linear tasks via learned kernels.
\end{quote}
Matched (level 4): \emph{Transformers Can Implement Preconditioned Richardson Iteration for In-Context Gaussian Kernel Regression} (arXiv, May 2026).

\textbf{C1 (generalist), realized at level 3 in a tier-1 venue.}
\begin{quote}\small\itshape
I propose latency-targeted threshold learning: insert layer-wise learnable pruning thresholds and jointly optimize weights and thresholds with a differentiable latency objective built from device-specific per-layer LUTs. A Lagrangian enforces a user-specified speedup during training, while structured sparsity (e.g., channel and GPU 2:4) is induced via relaxed top-k/straight-through mask parametrizations and then discretized at the end to preserve measured latency. This unifies speed-constrained allocation with end-to-end sparse training, aiming to deliver the hardware-guaranteed speedups of post-training solvers without the accuracy loss from late pruning. Empirically, this should match or surpass post-hoc speed-aware pruning at equivalent targets while eliminating extra retraining passes.
\end{quote}
Matched (level 3): \emph{LightPrune: Latency-Aware Structured Pruning for Efficient Deep Inference on Embedded Devices} (ICCV, October 2025).

\textbf{C4 ($m{=}800$), not (yet) realized.}
\begin{quote}\small\itshape
We propose RKHS-Hopfield Linear ICL: parameterize the attention kernel feature map so that linearized attention implements a proximal/gradient step in a reproducing-kernel Hilbert space tailored to the task family, thereby shaping the Hopfield energy landscape to encode desirable attractor basins for in-context learning. Concretely, we learn kernel features and value projections so a single attention update matches a functional gradient descent step on the implicit RKHS loss given the context, with multi-heads covering different step sizes or preconditioners. This yields interpretable, stable associative retrieval aligned with gradient-based learning while retaining linear-time, streaming updates for long contexts. We expect improved non-linear ICL and better extrapolation by aligning the learned kernel with the task NTK and training via a meta-objective that penalizes deviation from the target functional update.
\end{quote}

\textbf{C0 (topic name only), not realized.}
\begin{quote}\small\itshape
Current self-supervised video representations entangle content and dynamics, making them hard to steer along meaningful control dimensions like pose, speed, or viewpoint without labels. Our key insight is to treat controllability as learning a latent operator algebra: mine naturally occurring near-duplicate video segments where only one factor changes, and use their residual differences as pseudo-interventions that reveal control directions. We propose Latent Control Algebra, which retrieves such pairs/triples, estimates dense correspondences to isolate residual changes, factorizes them into a small set of latent generators, and trains an encoder--decoder where content codes plus exponentiated generators reconstruct targets under cycle/inverse consistency and commutator regularization for disentanglement; rolling out generators must also predict short-horizon futures. This yields self-supervised, composable control axes that enable latent steering, controllable editing, and zero-shot policy transfer in downstream tasks.
\end{quote}

\textbf{No-search ablation, not realized.}
\begin{quote}\small\itshape
Adaptive Bias--Variance Compression (ABVC) for federated optimization tunes, per client and round, the bias--variance operating point of a generalized compressor to minimize bits-to-accuracy under heterogeneity and partial participation. ABVC combines EF-BV's error feedback with variance reduction via gradient-difference tracking in the MARINA style, and augments updates with a lightweight Hessian-learning module to form diagonal/low-rank preconditioners that set client-specific compression budgets; once the preconditioner stabilizes, we enable Nesterov-style acceleration as in ACGD. We hypothesize linear convergence under PL/strongly convex objectives with rates that smoothly interpolate between unbiased-VR and biased-contractive regimes, and improved scaling with the number of workers due to independent randomness and preconditioned error dynamics. Empirically, on vision/NLP FL benchmarks with top-k and low-rank compressors, ABVC should reduce communication at fixed accuracy and time-to-$\varepsilon$ versus EF-BV, MARINA, and FedNL variants.
\end{quote}
...
\subsection{Realization-level distributions}
\label{app:leveldist}

Figure~\ref{fig:leveldist} breaks every dial condition down by best
realization level, rather than by the acc@$\geq$2 threshold reported in the
main text, with one panel per recognition tier.

\begin{figure}[H]
\begin{center}
\includegraphics[width=0.62\linewidth]{dial-levels-tier1.png}\\
\includegraphics[width=0.62\linewidth]{dial-levels-tier2.png}\\
\includegraphics[width=0.62\linewidth]{dial-levels-tier3.png}
\end{center}
\caption{Idea counts by best realization level for each dial condition,
one panel per recognition tier (stacked bars; bar height = total ideas
generated). Level 1 (``same broad area, different goal'') is never the
best match in any tier---the judge's effective scale is 0/2/3/4/5.}
\label{fig:leveldist}
\end{figure}
