...
tier-1 accuracy 31\% vs.\ 14\% for unrestricted generalists, with the
whole narrow band (100--800) well ahead of broad scopes; (ii) idea
...
generalists are C1 ($m=\infty$). All
...
writes are always free. Only realizations published on/after 2025-06-01
count (Sec.~\ref{sec:eval}). Each tier cell reports acc@$\geq$2, the
number of realized ideas (level $\geq 2$, in parentheses), and the mean
best realization level.}
\label{tab:dial}
\begin{center}\small
...
when the window is applied (Sec.~\ref{sec:noedges}). Two residual
risks remain: the window is behavioral evidence, not a provider
guarantee, and a paper published in-window may have circulated as a
preprint earlier; both bias reported accuracies in opposite directions
and neither affects between-condition comparisons, which all use the
same window.
