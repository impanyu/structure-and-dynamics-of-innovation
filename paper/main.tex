...
(and links) back into it. Agents propose ideas freely; each new idea is then
...
cutoff. A match counts only if that paper attacks the same specific problem
and appeared at least eight months past the cutoff---probe tests show the
model's memory runs well past its official date.
...
papers---about one sub-area---at 31\%, while both extremes sink to the same
floor, agents given no papers (13\%) doing no better than agents free to read
all 16{,}208 (14\%). The number of ideas produced follows the same curve.
...
edges makes generalists better (27\% vs.\ 14\%), while removing semantic
...
\textbf{Findings.}
\begin{itemize}
\item \textbf{An inverted-U in idea quality, peaking at a sub-area
  scope (Sec.~\ref{sec:dial}).} Tier-1 acc@$\geq$2 peaks at $m{=}800$
  (31\% vs.\ 14\% for generalists), with the whole narrow-to-mid band
  $m \in \{100, 400, 800, 1600\}$ (24--31\%) well above broad scopes
  (15--19\%) and generalists last. The pre-registered fear that very
  narrow scopes would starve agents of recombination material did not
  materialize---but the narrow end's advantage over the peak appears
  only under memory-contaminated scoring, not under the guarded
  metric. Both endpoints of the dial sit on the \emph{parametric
  floor}: agents that read nothing (topic name only, 13\%) and
  agents that may read everything (14\%) are indistinguishable---
  reading pays only when it is narrow.
\item \textbf{Quantity peaks at mid-narrow scope
  (Sec.~\ref{sec:dial}).} Scoped readers commit to ideas sooner and
  produce more of them (83 at $m{=}400$ vs.\ 50 for generalists over
  the same 80 rounds; 67 at $m{=}100$); generalists spend their actions
  wandering and curating.
\item \textbf{Team size is a throughput knob, not a quality knob
  (Sec.~\ref{sec:scaling}).} From $N=10$ to $N=150$ generalists at fixed
  depth, per-idea accuracy is flat (12--19\%) while absolute hits grow
  roughly linearly. The mechanism is stark: agents are effectively independent.
  Across all scaling runs (575 ideas, 10{,}300 actions) not one agent
  cited another agent's generated idea.
\item \textbf{Navigation channels have three distinct roles
  (Sec.~\ref{sec:noedges}).} Deleting every initial citation edge makes
  generalists \emph{better} (60 ideas at 27\% tier-1 accuracy vs.\ 50
  at 14\%): deprived of neighbors to wander through, agents commit to
  pure semantic-search navigation and even rebuild structure by their
  own logic. Disabling random jumps is neutral. Disabling semantic
  search quadruples production but \emph{halves} guarded accuracy
  (10\%)---search is where genuine anticipation comes from; without
  it, agents backfill from parametric memory, which the realization
  window discounts.
...
\textbf{tier~3} admits any published paper.
