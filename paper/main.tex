...
corpus. We find (i) anticipation accuracy \emph{rises as reading
narrows}: the tightest scope---100 papers, the inner core of one
topic---is best on every tier (tier-1 accuracy 51\% vs.\ 20\% for
unrestricted generalists), and every scoped condition beats the
generalists; (ii) idea production peaks at a mid-narrow scope and falls
as reading widens; (iii) team size is a pure throughput
...
\textbf{Findings.}
\begin{itemize}
\item \textbf{Idea quality rises as reading narrows
  (Sec.~\ref{sec:dial}).} Per-idea anticipation accuracy grows almost
  monotonically as scope shrinks, and the narrowest scope wins: at
  $m{=}100$---a region \emph{smaller} than the median natural topic
  (228 papers), i.e., one topic's inner core---tier-1 acc@$\geq$2
  reaches 51\% vs.\ 20\% for generalists, with the best mean
  realization level in every tier. Every scoped condition beats the
  unrestricted generalists; the pre-registered fear that very narrow
  scopes would starve agents of recombination material did not
  materialize.
...
writes are always free. Each tier cell reports acc@$\geq$2, the number of
realized ideas (level $\geq 2$, in parentheses), and the mean best
realization level.}
\label{tab:dial}
\begin{center}\small
...
\midrule
C2 & 100   & 67 & \textbf{51\% (34) / 1.24} & \textbf{55\%} (37) / 1.37 & 79\% (53) / 2.13 \\
C3 & 400   & \textbf{83} & 36\% (30) / 0.89 & 47\% (39) / 1.19 & \textbf{89\% (74)} / \textbf{2.48} \\
C4 & 800   & 74 & 41\% (30) / 1.04 & 53\% (39) / \textbf{1.38} & 84\% (62) / 2.43 \\
C5 & 1600  & 67 & 30\% (20) / 0.78 & 39\% (26) / 1.04 & 60\% (40) / 1.60 \\
C6 & 3200  & 55 & 27\% (15) / 0.69  & 40\% (22) / 1.05 & 73\% (40) / 2.02 \\
C7 & 6400  & 54 & 30\% (16) / 0.74  & 41\% (22) / 1.06 & 78\% (42) / 2.22 \\
C1 & $\infty$ & 50 & 20\% (10) / 0.50 & 26\% (13) / 0.64 & 62\% (31) / 1.68 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
\textbf{Narrower is better.} Tier-1 accuracy runs \textbf{51\%}
$\to$ 36\% $\to$ 41\% $\to$ 30\% $\to$ 27\% $\to$ 30\% $\to$ 20\% as
scope widens from 100 papers to the whole corpus; mean realization
level tracks the same shape in every tier. The narrowest scope
($m{=}100$, smaller than the median natural topic of 228 papers) is
best on accuracy and mean level in tiers 1--2 and on absolute tier-1
hits (34), refuting the pre-registered expectation of material
starvation at the narrow end; the $m{=}400$ dip below $m{=}800$ is
within single-seed noise. Broad scopes underperform throughout
(unfocused wandering; a semantically diffuse topic region produced an
agent with zero output).
...
almost no re-reading of the agent's own output. Within the standard regime, then, search-fed material predicts
realization while link-fed material does not---consistent with the
edge-removal ablation of Sec.~\ref{sec:noedges}. The search-removal
ablation there adds a caution against a purely causal reading: without
search, agents recover unchanged per-idea accuracy by other routes, so
search-fed reading marks the deliberative mode rather than being the
sole conduit of quality.
...
initial citation edges, random jumps, or semantic search. Removing any
channel increases output without hurting per-idea accuracy; the full
action set is the \emph{worst} configuration.}
\label{tab:noedges}
\begin{center}\small
\begin{tabular}{lrccc}
\toprule
 ...
 \textbf{tier-2 acc / mean} & \textbf{tier-3 acc / mean} \\
\midrule
no edges & 60 & \textbf{37\% (22) / 0.93} & \textbf{43\%} (26) / \textbf{1.07} & 75\% (45) / 1.97 \\
no jumps & 63 & 24\% (15) / 0.62 & 27\% (17) / 0.71 & 73\% (46) / 1.92 \\
no search & \textbf{201} & 24\% (48) / 0.59 & 36\% (73) / 0.88 & \textbf{76\% (152)} / \textbf{2.00} \\
C1 (full) & 50 & 20\% (10) / 0.50 & 26\% (13) / 0.64 & 62\% (31) / 1.68 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
jumps points the same way with a smaller effect (63 ideas at 24\%
vs.\ 50 at 20\%)---unsurprising, since C1 agents only jumped $\sim$20
times in 800 actions. Disabling \emph{semantic search} produces the
most dramatic behavioral shift of any intervention in this paper:
production quadruples (201 ideas) while per-idea accuracy holds at
24\%, yielding 48 tier-1 hits. Search is thus a \emph{throttle}, not
the sole source of quality: it is the channel that keeps agents in
deliberation (``keep reading'') mode, and without it they write
prolifically from links, jumps, and parametric knowledge at unchanged
per-idea quality. Together the three ablations give a stark summary:
for unrestricted generalists, removing \emph{any} navigation channel
helps, and the full action set is the worst configuration---the
generalist's failure mode is wandering, in whichever channel permits
it. We caution that these are single seeded runs per arm; the
...
idea quality is \emph{how reading is situated}---the narrower the
reading, the better, down to a single topic's hundred-paper core, and
any scope beats none---while team size sets only throughput, because
today's agents do not yet read each other. The human citation structure itself,
