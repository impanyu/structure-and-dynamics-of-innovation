...
\emph{helps} generalists (tier-1 accuracy 20\% vs.\ 10\%, with more ideas
...
\item \textbf{The citation structure is a distractor, not a scaffold
  ...
  generalists \emph{better}: 60 ideas at 20\% tier-1 accuracy vs.\ 50 at
  10\% with edges, under otherwise identical conditions. Deprived of
...
\begin{table}[t]
\caption{Initial-graph ablation: identical to C1 (10 generalists, 80
rounds, same seed and corpus) except the initial network carries no
citation edges. Removing the structure improves both output and
accuracy.}
\label{tab:noedges}
\begin{center}
\begin{tabular}{lrrrrr}
\toprule
 & \textbf{ideas} & \textbf{tier-1 acc (hits)} & \textbf{tier-2} &
 \textbf{tier-3} & \textbf{tier-1 mean lvl} \\
\midrule
no edges & \textbf{60} & \textbf{20\% (12)} & \textbf{20\%} & \textbf{47\%} & \textbf{0.93} \\
C1 (80{,}962 edges) & 50 & 10\% (5) & 12\% & 44\% & 0.50 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
more ideas at double the tier-1 accuracy---approaching the dial's
$m{=}800$ optimum---and starts generating at team round 8 instead of 17.
...
corpus's citation logic. We caution that this is a single seeded run per
arm (12/60 vs.\ 5/50 hits); the direction, the mean-level gap, and the
behavioral signature are mutually consistent, but replication is needed
before the strongest reading.
